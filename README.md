...
- **取件码卡片视图**：取件码分组以卡片形式展示每条通知——第一行大号加粗取件码、第二行快递公司名（中通/圆通/韵达/极兔…）、第三行取件点地址（如「某小区X栋X室」/「菜鸟驿站」），分级清晰、一眼定位。
...
| 首页 | 分组详情 |
| --- | --- |
| ![首页](screenshots/home.png) | ![取件码分组（卡片视图，含时间与图标）](screenshots/group.png) |
...
- 下载：<https://github.com/wq198155/NotifyCollector/releases> 获取最新 `NotifyCollector-1.10.apk`（已签名，可直接侧载）
- 大小：约 11.8 MB（含钉钉进步体字体 2.1 MB）
- SHA-256：`f6efcf87e6f921ab2b700696f343997877ae5db2b3e6524e4330dba6a3a0de43`
...
- v1.10：地址解析边缘用例加固。补充「放置于」起点词；尾部噪声增加「请/代收/代取」（修复无标点短信「…便利店**请**凭码取件」残留「请」）；新增主语剥离，修复无起点标记短信「您的快递**已由**丰巢柜**代收**」整句被当地址。回归测试扩到 21 条样本
- v1.9：公司名改为取正文中**出现位置最靠前**的关键词（原来按关键词表顺序取，导致「您的快递中通，已到…室顺丰」被误判为顺丰）
- v1.8：取件码卡片地址改为**取到句子结束**。原实现遇到「室」就截断，导致「…8栋101室顺丰」被砍成「…8栋101」；同时收紧「号」为「数字+号」（避免银行卡短信「尾号1234」误判），并加「必须含强地址特征」校验
- v1.7：首页左滑「编辑」去掉二次确认框，直达编辑页（仅「删除」保留二次确认）；取件码卡片复制按钮移到取件码后面（时间改到卡片右上角单独成行）；取件码字号加大到 headlineLarge
- v1.6：取件码卡片「复制」按钮，点击把取件码写入剪贴板
- v1.5：取件码匹配规则移除「快捷」歧义关键词（避免「快捷支付」类通知被误抓）；展示用的公司名表同步移除「快捷」，避免正文含「快捷」二字的通知被误标为快捷快递
