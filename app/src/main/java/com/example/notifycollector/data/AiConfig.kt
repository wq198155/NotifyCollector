...
object AiConfig {
    ...
    fun buildPrompt(title: String, text: String): String = buildString {
        ...
        appendLine("字段说明：")
        appendLine("- category: 最匹配的分类，取值之一：取件码 / 验证码 / 银行动账 / 快递物流 / 其他（含取件码或取件号的通知归取件码；纯物流状态更新如已揽收/运输中/派送中/已签收/派件归快递物流）")
        appendLine("- code: 取件码或验证码的数字或字母串，没有则为空字符串")
        appendLine("- company: 快递或承运公司名，没有则为空字符串")
        appendLine("- location: 取件地址，取到房间号/楼栋/驿站为止，没有则为空字符串")
        appendLine("- confidence: 你对以上判断的把握，0 到 1 之间的小数")
        appendLine("只根据通知真实内容填写，通知中没有的字段一律填空字符串，不要编造。")
        appendLine()
